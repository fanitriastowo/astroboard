defmodule Astroboard.Accounts.UserToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  # Reset links also grant account takeover, so keep them short-lived too.
  ...
  If found, the query returns the user.

  The given token is valid if it matches its hashed counterpart in the
  database, has not expired (after @reset_password_validity_in_hours) and
  was sent to the user's current email. The context of a reset password
  token is always "reset_password".
  """
